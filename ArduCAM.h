...
#define fontunsigned char(x) pgm_read_unsigned char(&cfont.font[x])  
...
#define swap(type, i, j) {type t = i; i = j; j = t;}
#define fontunsigned char(x) cfont.font[x]  
...
#define pgm_read_unsigned char(x)        (*((char *)x))
#define pgm_read_word(x)        ( ((*((unsigned char *)x + 1)) << 8) + (*((unsigned char *)x)))
#define pgm_read_unsigned char_near(x)   (*((char *)x))
#define pgm_read_unsigned char_far(x)    (*((char *)x))
...
class ArduCAM 
{
	public:
	ArduCAM( void );
	ArduCAM(unsigned char model, int CS);
	void InitCAM( void );
  ...
  // Write 16 bit values to 16 bit register address
	...
	unsigned char rdSensorReg8_8(uint8_t regID, uint8_t* regDat);
	
	// Read/write 16 bit value to/from 8 bit register address
	unsigned char wrSensorReg8_16(int regID, int regDat);
	unsigned char rdSensorReg8_16(uint8_t regID, uint16_t* regDat);
	
	// Read/write 8 bit value to/from 16 bit register address
	unsigned char wrSensorReg16_8(int regID, int regDat);
	unsigned char rdSensorReg16_8(uint16_t regID, uint8_t* regDat);
	
	// Read/write 16 bit value to/from 16 bit register address
	unsigned char wrSensorReg16_16(int regID, int regDat);
	unsigned char rdSensorReg16_16(uint16_t regID, uint16_t* regDat);
  ...
  void OV5640_set_Banding_Filter(uint8_t Banding_Filter);
	...
	void transferunsigned chars_(uint8_t * out, uint8_t * in, uint8_t size);
	void transferunsigned chars(uint8_t * out, uint8_t * in, uint32_t size);
